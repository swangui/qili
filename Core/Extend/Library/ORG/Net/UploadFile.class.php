...
class UploadFile {//类定义开始
    ...
    private $config =   array(
		...
		'imageQuality'      =>  90,  // 图片品质
		'imageMaxWidth'     =>  '800',//图片的最大宽度，广告图片需改为1920
		'imageMaxHeight'    =>  '650',//图片的最大高度,不用更改  
...
/**********杨涛修改以下**********/
		$image=$file['tmp_name'];
		$attribute = getimagesize($image); // $attribute[0]为图片高度,[1]为图片宽度,[2]为图片类型:如果输出值为2的话就代表是jpg;
		switch($attribute[2]) {
		...
		//图片宽度大于设定宽度时，并且宽度大于高度
		if(($attribute[0] > $this->imageMaxWidth) && ($attribute[0] > $attribute[1])) {
		    $width = $this->imageMaxWidth;
		    $height = (integer) ($attribute[1]/(round($attribute[0]/$width,2)));
		//图片高度大于设定高度时，并且高度大于宽度
		}else if(($attribute[1] > $this->imageMaxHeight) && ($attribute[1] > $attribute[0])) {
		    $height = $this->imageMaxHeight;
		    $width = (integer) ($attribute[0]/(round($attribute[1]/$height,2)));			
		//图片为正方形时，并且高度大于设定高度
		}else if(($attrobite[0] == $attrobite[1]) && ($attrobite[1] >$this->imageMaxHeight)) {
		    $height = $this->imageMaxHeight;
			$width = $height;
	    }else {
		    $width = $attribute[0];
		...
		imagecopyresampled($new, $im, 0, 0, 0, 0, $width, $height, $attribute[0], $attribute[1]);
		switch($attribute[2]){//utf8模式下输出中文名会乱码所以输入一定是英文名或数字;
		  case 1:imagegif($new,$filename);break;//主要为了考虑广告图片
		  case 2:imagejpeg($new,$filename,$this->imageQuality);break;//imagejpeg有3个参数，第3个参数是质量，默认是75，所以图片上传后被压缩了
		  case 3:imagejpeg($new,$filename,$this->imageQuality);break;//png也是用jpeg输出，因为imagepng输出文件太大了
		  default:imagejpeg($new,$filename,$this->imageQuality);break;
		}
		imagedestroy($new);//销毁图像标识符，回收占用的资源				
